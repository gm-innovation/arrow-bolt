...
import { GraduationCap, BookOpen, Award, Play, Download, Loader2, Eye } from 'lucide-react';
...
interface Props {
  /** Where course cards navigate to ("/corp/university/course" or "/hr/university/course") */
  courseHrefBase?: string;
  /** Show the page-level title and icon. Set to false when embedded inside another page. */
  showHeader?: boolean;
}

const MyLearningPanel = ({ courseHrefBase = '/corp/university', showHeader = true }: Props) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="active" storageKey="my-learning-panel">
        <TabsList>
          ...
          {loadingEnrollments ? <p className="text-muted-foreground py-4">Carregando...</p> : !activeEnrollments.length ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {activeEnrollments.map(e => {
                ...
                return (
                  <Card key={e.id} className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => navigate(`${courseHrefBase}/course/${e.course_id}`)}>
                  ...
                  <CardContent className="pt-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex gap-1">
                        <Button size="sm" variant="outline" onClick={() => handlePreview(cert)} disabled={previewLoading === cert.id}>
                          {previewLoading === cert.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Eye className="h-4 w-4" />}
                        </Button>
                        <Button size="sm" variant="outline" onClick={() => handleDownload(cert)} disabled={downloadingId === cert.id}>
