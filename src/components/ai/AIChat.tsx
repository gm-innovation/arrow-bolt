...
export function AIChat({ userRole, agentName = 'Arrow AI', avatarUrl, context }: AIChatProps) {
  ...
  // Toda a mecânica de áudio vive atrás do contrato VoiceSession.
  // Ver src/lib/voice/FULL-DUPLEX.md para o ponto de troca do transporte.
  const { compactSession } = useVoiceTelemetry();
  const [liveOn, setLiveOn] = useState(false);

  const lastAssistant = messages[messages.length - 1];
  const assistantStream =
    liveOn && lastAssistant?.role === 'assistant'
      ? { key: lastAssistant.id ?? `live-${messages.length}`, content: lastAssistant.content ?? '' }
      : null;

  const voice = useVoiceSession({
    isThinking: isLoading,
    assistant: assistantStream,
    speechOptions: voiceOpts,
    conversationId: currentConversationId,
    onUtterance: (text) => sendMessage(text, undefined, { channel: 'voice' }),
    onTurnComplete: () => {
