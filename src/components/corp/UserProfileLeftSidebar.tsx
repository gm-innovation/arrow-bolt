...
import { Users, Award, FileText, Image, Film, FileIcon } from 'lucide-react';
...
const UserProfileLeftSidebar = ({ targetUserId }: Props) => {
  ...
  // Recent posts with attachments
  const { data: recentPosts = [] } = useQuery({
    queryKey: ['user-recent-posts', targetUserId],
    queryFn: async () => {
      const { data } = await supabase
        .from('corp_feed_posts')
        .select('id, content, title, created_at, corp_feed_attachments(file_url, file_type, file_name, mime_type)')
  ...
  const getAttachmentIcon = (attachment: any) => {
    ...
    if (type.startsWith('image') || type === 'image') return <Image className="h-3 w-3 text-blue-500" />;
    if (type.startsWith('video') || type === 'video') return <Film className="h-3 w-3 text-purple-500" />;
          ...
          {recentPosts.length > 0 ? (
            <div className="space-y-2.5">
              {recentPosts.map((post: any) => {
                ...
                const imageAtt = attachments.find((a: any) => (a.file_type || a.mime_type || '').includes('image'));
                return (
                  <div key={post.id} className="border-b border-border pb-2.5 last:border-0 last:pb-0">
                    {imageAtt && (
                      <img src={imageAtt.file_url} alt="" className="w-full h-20 object-cover rounded mb-1.5" />
                    )}
                      ...
                      {attachments.length > 0 && (
                        <div className="flex items-center gap-0.5">
                          {attachments.slice(0, 3).map((a: any, i: number) => (
                            <span key={i}>{getAttachmentIcon(a)}</span>
                          ))}
