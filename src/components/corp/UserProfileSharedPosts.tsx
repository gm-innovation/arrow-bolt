...
import { Share2, Image, Film, FileIcon } from 'lucide-react';
...
const UserProfileSharedPosts = ({ targetUserId }: Props) => {
  const navigate = useNavigate();

  // Fetch posts where user is mentioned
  const { data: sharedPosts = [], isLoading } = useQuery({
    queryKey: ['user-shared-posts', targetUserId],
    queryFn: async () => {
      // Get mentions for this user
      const { data: mentions } = await supabase
        .from('corp_feed_mentions')
        .select('post_id')
        .eq('mention_type', 'user')
        .eq('mention_value', targetUserId);

      if (!mentions?.length) return [];

      const postIds = [...new Set(mentions.map(m => m.post_id))];
    ...
    <Card>
      <CardContent className="p-4">
        <h3 className="text-sm font-semibold flex items-center gap-1.5 mb-4">
          <Share2 className="h-4 w-4 text-primary/60" /> Publicações compartilhadas comigo
        </h3>
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div>
                      <p className="text-sm font-medium cursor-pointer hover:text-primary transition-colors"
                        onClick={() => navigate(`/corp/profile/${post.author_id}`)}>
...
export default UserProfileSharedPosts;
