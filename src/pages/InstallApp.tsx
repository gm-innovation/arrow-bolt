...
import { Smartphone, Download, Bell, Wifi, Zap, CheckCircle, Loader2 } from "lucide-react";
...
const InstallApp = () => {
  ...
  const getNotificationStatus = () => {
    if (isSubscribed && permission === 'granted') {
      return { label: 'Ativadas', variant: 'default' as const };
    }
    if (permission === 'denied') {
      return { label: 'Bloqueadas', variant: 'destructive' as const };
    }
    return { label: 'Desativadas', variant: 'secondary' as const };
  };
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {!vapidConfigured && (
            <Alert>
              <AlertDescription>
                As notificações push estão sendo configuradas. Por favor, tente novamente em alguns instantes.
              </AlertDescription>
            </Alert>
          )}
          ...
          {isSupported && permission !== 'granted' && vapidConfigured && (
            <Button 
          ...
          {permission === 'granted' && !isSubscribed && vapidConfigured && (
            <Button 
              onClick={handleEnableNotifications} 
              variant="outline"
              className="w-full"
